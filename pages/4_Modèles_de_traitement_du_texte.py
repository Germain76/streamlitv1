...
ml_models, dl_models, demo, lstm = st.tabs(["Machine learning", "Deep learning", "Démo", "Modèle retenu"])

with ml_models:
    
    st.markdown("""Pour classifier le texte, nous avons d'abord testé plusieurs **modèles de Machine Learning**. Nous avons testé les modèles suivants : """)
    st.markdown(""" 
                - LinearSVC
                - SVC
                - Logistic Regression
    ...
    st.write("Voici les résultats obtenu par les meilleurs modèles. Les modèles souffrent un petit peu d'overfitting, mais obtiennent globalement un bon score sur le texe nettoyé.")
...
with demo:
    # Live testing
    st.subheader("Démo")
    st.write("Voici un widget permettant d'afficher les différentes étapes de la **pipeline de nettoyage et traitement du texte**, puis essayant de prédire la classe du produit à l'aide du classifieur **LinearSVC** et du modèle **LSTM**.")
    # Widget
    with st.container(border=True):
        # Load model button
        ...
        # If model is loaded
            ...
            line = backend.load_raw_df().loc[st.session_state.selected_line, :] # Get line from raw dataframe
            line = line.fillna(value="")
            with st.container():
                with st.expander(f"Longeur totale : {len(str(line['description'])+str(line['designation']))} caractères"):
...
with lstm:
    st.markdown("Les modèles de machine learning affichaient des résultats supérieurs, mais ils n'étaient **pas compatibles** avec l'utilisation de la classe `tf.data.Dataset`. Cette classe étant nécessaire pour pouvoir entrainer le classifieur multimodal, nous avons fait le choix d'un modèle **un peu moins performant initialement**, pour pouvoir construire un **modèle final plus performant** que le LinearSVC, le meilleur modèle de machine learning.")
    st.markdown("Nous avons donc finalement retenu le modèle **LSTM** pour comme modèle final.")
